...
# Go `database/sql` (with Postgres)
The Go standard library's `database/sql` package provides a simple API for managing a database driver. To use with a Postgres database driver:
...
import (
    ...
    // Import a driver specific to the database vendor.
    // Be sure to 'go get' the package beforehand.
    // Even if not explicitly called, it must be imported
    // to initialize the driver.
    _ "github.com/lib/pq"

// The Postgres driver requires variables to build a datasource.
const (
	host     = "localhost"
	port     = 5432
	user     = "postgres"
    ...
    func main() {
        ...
        // Open a connection using sql.Open, passing the name of
        // the driver provided and its datasource args
        db, _ := sql.Open("postgres", datasource)

        // Executing sql commands
        db.Exec("INSERT INTO mytable VALUES (1, 'mehrab')")
        db.Exec("INSERT INTO mytable VALUES (2, 'rahman')")

        // Read one row from a query
        row := db.QueryRow("SELECT * FROM mytable WHERE id = $1", "mehrab")

        // Parse returned row values
        var id int
        var name string
        row.Scan(&id, &name)
    }
